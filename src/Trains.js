...
export default class Trains extends Component {
  ...
  current() {
    const grouped = groupby(
      this.props.result.TrainAnnouncement,
      "AdvertisedTrainIdent"
    );
    const object = filter(map(grouped, announcementsToObject), "actual");
    const sorted = this.sortTrains(
      object,
      direction(this.props.result.TrainAnnouncement),
      this.props.stations
    );
  ...
  sortTrains(object, dir) {
    return orderby(
      object,
      [
        a => this.north(a.actual.LocationSignature, this.props.stations),
        "actual.ActivityType",
  ...
  between(loc1, loc2) {
    return (
      (wgs.north(loc1, this.props.stations) +
        wgs.north(loc2, this.props.stations)) /
      2
    );
  }
